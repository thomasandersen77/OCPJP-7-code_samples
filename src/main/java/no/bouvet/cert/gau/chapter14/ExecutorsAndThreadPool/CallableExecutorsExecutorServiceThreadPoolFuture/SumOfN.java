package no.bouvet.cert.gau.chapter14.ExecutorsAndThreadPool.CallableExecutorsExecutorServiceThreadPoolFuture;
...
// We divide the task to sum the numbers to 10 threads (Which is an arbitrary limit
// just for illustration). Once computation is complete, we add the results of all
// the threads, and check if the calculation is correct by using the formula (N*(N + 1))/2.
public class SumOfN {
    ...
    // In the main method we implement the logic to divide the summation task to
    // given number of threads and finally check if the calculated sum is correct
    public static void main(String[] args) {
        // Divide the task among available fixed numbers of threadss
        ExecutorService executorService = Executors.newFixedThreadPool(NUM_THREADS);
        // store the references to the Future object in a List  for summing up together
        ...
        for(int i = 0; i<NUM_THREADS;i++){
            ...
            System.out.printf("Spawning thread from summing in range %d to %d %n",
        ...
        for(Future<Long> partialSum : summationTasks){
            ...
            } catch (CancellationException | InterruptedException | ExecutionException e) { //multi catch
