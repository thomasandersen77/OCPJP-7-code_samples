package no.bouvet.cert.gau.chapter14.ForkJoin;
...
public class SumOfNUsingForkJoin {
    ...
    private static final int NUM_THREADS = 10; //number of threads created for distributing the effort

    // This is the recursive implementation of algorithm; inherit from RecursiveTask
    // instead of RecursiveAction because we're returning values
    static class RecursiveSumOfN extends RecursiveTask<Long>{
        ...
        // (remember that we want to divide the summation task equally amount NUM_THREADS)
        ...
        protected Long compute() {
            if( (to - from) <= N/NUM_THREADS){
                ...
                long localSum = 0;
                // add in range 'from' .. ' to' inclusive the value 'to'
                for(long i = from; i<=to; i++){
                    localSum += i;
                }
                System.out.printf("\t Summing of value range %d is %d %n",
                        from,to,localSum);
                return localSum;
            }
            else{
